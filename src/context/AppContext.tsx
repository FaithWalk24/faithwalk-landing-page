...
// Initial sample customer downloads
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [activeAppExperience, setActiveAppExperience] = useState<('faithwalk-daily' | 'faithwalk-companion') | null>(null);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('faithwalk_subscriptions', JSON.stringify(subscriptions));
    } catch {
      // ignore
    }
  }, [subscriptions]);
  ...
  const subscribeToApp = (appId: 'faithwalk-daily' | 'faithwalk-companion') => {
    ...
    // If not logged in, log in with default demo session so the user can immediately see it in Personal Sanctuary
    if (!isLoggedIn) {
  ...
  const isSubscribed = (appId: 'faithwalk-daily' | 'faithwalk-companion') => {
    const sub = subscriptions.find((s) => s.appId === appId);
    return sub ? sub.status === 'active' : false;
  };

  const openAppExperience = (appId: 'faithwalk-daily' | 'faithwalk-companion') => {
    setActiveAppExperience(appId);
  };
